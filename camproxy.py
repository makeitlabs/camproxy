...
SSLCertFile = Config.get('Flask', 'SSLCertFile')
SSLKeyFile = Config.get('Flask', 'SSLKeyFile')
        ...
        if '@'+RequiredDomain in ppl_doc['emailAddresses'][0]['value']:
          if cam:
            if '-' in cam:
              ...
              h = httplib2.Http()
              userpass = str(NXUser) + ':' + str(NXPassword)

              auth = base64.b64encode(userpass.encode('utf-8'));
              resp, content = h.request(URL, 'GET', headers = { 'Authorization' : 'Basic ' + auth.decode('utf-8') })

              return Response(content, mimetype='image/jpeg')

            else:
              return "Invalid camera hash format"
